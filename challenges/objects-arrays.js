...
for(i = 0; i < graduates.length; i++){
  var item = graduates[i];
  var keys = object.keys(item);
  var values = object.values(item);
  for (a = 0; a < graduates.length; a++){
    var key = keys[a];
    var value = values[a];
    if(key == 'university'){
...
const uni = [];
for(b = 0; b < graduates.length; b++){
  if(graduates[i].match("uni")){
    uni.push(i);
  }
} 
console.log(uni);
...
*/
const animalNames = [];
animalNames = zooAnimals.array.forEach(function(object){
    animalNames.push(`${object.animal_name} ${object.scientific_name}`);  
});
...
largerPopulation = zooAnimals.filter((object) => {
...
populationTotal = zooAnimals.reduce((value, population) => {
    return value += population.population;
});
